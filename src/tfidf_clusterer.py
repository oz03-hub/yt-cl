...
class TfidfClusterer(BaseClusterer):
    ...
    def preprocess(self, df: pd.DataFrame) -> np.ndarray:
        X = self.vectorizer.fit_transform(df["transcript"])
        return X
    ...
    def calculate_metrics(self) -> Dict[str, float]:
        self.metrics = {
            ...
            "davies_bouldin_score": davies_bouldin_score(self.X.toarray(), self.labels_)
        }
        return self.metrics
    ...
    def get_cluster_centers(self, df: pd.DataFrame) -> str:
        ...
        df_with_vectors["tfidf_vector"] = [row for row in self.X.todense()]
        df_with_vectors["tfidf_vector"] = df_with_vectors["tfidf_vector"].apply(lambda x: np.array(x))

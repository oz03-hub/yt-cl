...
def run_n_clustering(n_clusters, X, model, random_state=None):
    ...
    if model == "tfidf":
        ...
        labels = embedding.cluster_from_x(X, n_clusters, random_state=random_state).labels_
    elif model == "lda":
        # X[0] is id2word, X[1] is corpus
        labels = lda.get_cluster_from_df(X[0], X[1], n_clusters, random_state=random_state)
    return labels
    ...
    for run in tqdm(range(runs), desc="Running clustering comparisons"):
        ...
        for i, model1 in enumerate(models):
            for j, model2 in enumerate(models):
                model1_labels = run_n_clustering(n_clusters, X_runs[i], model1, random_state=random_state)
                model2_labels = run_n_clustering(n_clusters, X_runs[j], model2, random_state=random_state)
